...
class CanvasErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.warn(
      "3D Canvas error caught, falling back to 2D:",
      error,
      errorInfo,
    );
  }
  ...
  return (
    <div className="flex flex-col items-center bg-white/20 border border-white/40 p-6 rounded-[28px] shadow-sm backdrop-blur-md flex-1">
      ...
      <div className="mt-4 text-center">
        <h4 className="text-sm font-extrabold uppercase text-primary tracking-wider">
          {label}
        </h4>
        <p className="text-2xl font-black text-slate-800 mt-1">
          {(volume || 0).toLocaleString()} ml
        </p>
...
export default function Dashboard() {
  ...
  const [aiHistory, setAiHistory] = React.useState([
    {
      role: "assistant",
      text: "Xin chào! Tôi là Trợ lý Kho hàng AI WebSen. Nhập tin nhắn tiếng Việt để điều khiển kho nhanh chóng. (Vd: 'nhập bồ kết 300ml 15 cái')",
    },
  ]);
  ...
  const handleSendAi = async (e, customText = null) => {
    ...
    try {
      ...
      setAiHistory((h) => [
        ...
        {
          role: "assistant",
          text: "Trợ lý đang bận xử lý dữ liệu. Vui lòng gửi lại sau.",
        },
      ]);
    } finally {
      setAiLoading(false);
  ...
  const fallback2D = (
    <div className="flex flex-col sm:flex-row gap-6 justify-center items-center max-w-3xl mx-auto py-8">
      <BottleFallback
        src="/boket.jpg"
        volume={boKetVolume}
        label="Bồ Kết Shampoo"
      />
      <BottleFallback
        src="/gung.jpg"
        volume={gingerVolume}
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          transform: `translate(${mouseCoords.x * 20}px, ${mouseCoords.y * 20}px)`,
        }}
      ...
      <div
        style={{
          transform: `translate(${mouseCoords.x * -15}px, ${mouseCoords.y * -15}px)`,
        }}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_0.5fr] max-w-[1700px] mx-auto">
        ...
        <div className="space-y-6">
          {/* Top Navbar details */}
          <div className="flex flex-col md:flex-row items-center justify-center md:justify-between gap-4 border-b border-slate-300/40 pb-4 text-center md:text-left">
            <div className="flex flex-col items-center md:items-start">
              <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0f5132] tracking-tight flex flex-wrap items-center justify-center md:justify-start gap-2">
                <span>WebSen</span>{" "}
                <span className="text-xs font-semibold uppercase bg-primary text-white px-2.5 py-1 rounded-full tracking-widest">
                  3D Real-time Inventory
                </span>
              </h1>
              <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mt-1 text-center md:text-left">
                Premium Herbal Shampoo Laboratory Dashboard
              </p>
            </div>
            <button
              onClick={fetchStats}
          ...
          <div
            className={`relative w-full h-fit min-h-[160px] md:h-40 rounded-[32px] border transition-all duration-1000 bg-white/40 shadow-soft backdrop-blur-md overflow-hidden p-4 md:p-6 select-none flex items-center ${slides[currentSlide].borderColor}`}
          >
            {/* Ambient sliding theme color overlays */}
            <div
              className={`absolute inset-0 bg-gradient-to-r ${slides[currentSlide].themeColor} opacity-70 transition-all duration-1000 -z-10`}
            />
            ...
            <div className="absolute right-[-10px] top-[-10px] text-7xl opacity-10 pointer-events-none select-none">
              🌿
            </div>
            <div className="absolute left-[35%] bottom-[-15px] text-5xl opacity-10 pointer-events-none select-none">
              🍃
            </div>
            ...
            <div className="w-full h-full relative overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="flex flex-col-reverse sm:flex-row justify-between items-center w-full gap-6 h-full relative z-10 text-center sm:text-left"
                >
                  {/* Animated Text Content */}
                  <div className="min-w-0 flex-1 space-y-2 flex flex-col items-center sm:items-start">
                    <span className="text-[10px] font-extrabold uppercase bg-primary/10 text-primary px-3 py-1 rounded-full tracking-widest inline-block select-none w-fit">
                      Sản phẩm nổi bật
                    </span>
                    <h2 className="text-base sm:text-xl md:text-2xl font-black text-slate-800 tracking-tight uppercase truncate max-w-full">
                  ...
                  <div className="shrink-0 relative flex justify-center w-full sm:w-auto">
                    <img
                      src={slides[currentSlide].image}
                      alt={slides[currentSlide].title}
                      className="h-20 w-20 sm:h-24 md:h-32 sm:w-24 md:w-32 rounded-2xl object-contain filter drop-shadow-lg bg-transparent transform"
                    />
                  </div>
                </motion.div>
              </AnimatePresence>
            ...
            {/* Indicator dots */}
            <div className="absolute left-1/2 -translate-x-1/2 sm:translate-x-0 sm:left-auto sm:right-6 bottom-4 flex gap-1.5 z-10">
              {slides.map((_, i) => (
                <span
                  key={i}
          ...
          <section className="relative rounded-[40px] border border-white/40 bg-[#FAF6F0]/45 p-6 md:p-8 shadow-soft backdrop-blur-md">
            ...
            <div className="absolute top-6 right-6 z-10 flex gap-2"></div>
            {/* Glow ring overlays when inventory updates */}
            <div
              className={`absolute inset-0 transition-opacity duration-700 rounded-[40px] border-2 border-emerald-500/40 shadow-2xl shadow-emerald-500/10 pointer-events-none -z-10 ${
                glowBoket ? "opacity-100" : "opacity-0"
              }`}
            />
            <div
              className={`absolute inset-0 transition-opacity duration-700 rounded-[40px] border-2 border-amber-500/40 shadow-2xl shadow-amber-500/10 pointer-events-none -z-10 ${
                glowGinger ? "opacity-100" : "opacity-0"
              }`}
            />
            <div className="text-center max-w-xl mx-auto mb-4">
              <span className="text-xs font-bold text-primary uppercase tracking-[0.22em]">
                3D Fluid Viscosity Simulation
              </span>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight mt-1">
                Trữ Lượng Dung Dịch Thực Tế
              </h2>
              <p className="text-xs text-slate-500 mt-2">
                Được dựng hình dạng chai thủy tinh PBR với mô phỏng chuyển động
                của chất lỏng đậm đặc (shampoo) trong thời gian thực.
              </p>
            </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2 max-w-3xl mx-auto mt-4 text-center border-t border-slate-300/30 pt-6">
              ...
              <div className="space-y-3 relative bg-white/30 border border-white/50 p-5 rounded-[24px] shadow-sm backdrop-blur-md">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center sm:items-baseline px-2 gap-2 text-center sm:text-left">
                  ...
                  <div className="text-center sm:text-right">
                    ...
                    <span className="text-xs font-bold text-primary ml-1.5 block sm:inline">
                      ({((boKetVolume || 0) / 1000).toFixed(1)} L)
                    </span>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-1 text-center text-xs px-2 py-2 bg-white/20 rounded-2xl mt-1 border border-white/30 backdrop-blur-sm">
                  <span className="text-[10px] sm:text-[11px] text-slate-600 font-semibold">
                    300ml: <br className="block sm:hidden" />
                    <b className="text-slate-900">
                      {stats?.boKetBottles300 || 0} chai
                    </b>
                  </span>
                  <span className="text-[10px] sm:text-[11px] text-slate-600 font-semibold">
                    500ml: <br className="block sm:hidden" />
                    <b className="text-slate-900">
                      {stats?.boKetBottles500 || 0} chai
                    </b>
                  </span>
                  <span className="text-[10px] sm:text-[11px] text-slate-600 font-semibold">
                    Xá: <br className="block sm:hidden" />
                    <b className="text-slate-900">
                      {stats?.boKetBulkLiters || 0} L
                    </b>
                  </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center px-1 text-center sm:text-left gap-1">
                  <span>
                    Fill Level:{" "}
                    <CountUp
                      to={boKetFill * 100}
              ...
              <div className="space-y-3 relative bg-white/30 border border-white/50 p-5 rounded-[24px] shadow-sm backdrop-blur-md">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center sm:items-baseline px-2 gap-2 text-center sm:text-left">
                  ...
                  <div className="text-center sm:text-right">
                    ...
                    <span className="text-xs font-bold text-amber-700 ml-1.5 block sm:inline">
                ...
                <div className="grid grid-cols-3 gap-1 text-center text-xs px-2 py-2 bg-white/20 rounded-2xl mt-1 border border-white/30 backdrop-blur-sm">
                  <span className="text-[10px] sm:text-[11px] text-slate-600 font-semibold">
                    300ml: <br className="block sm:hidden" />
                    <b className="text-slate-900">
                      {stats?.gingerBottles300 || 0} chai
                    </b>
                  </span>
                  <span className="text-[10px] sm:text-[11px] text-slate-600 font-semibold">
                    500ml: <br className="block sm:hidden" />
                    <b className="text-slate-900">
                      {stats?.gingerBottles500 || 0} chai
                    </b>
                  </span>
                  <span className="text-[10px] sm:text-[11px] text-slate-600 font-semibold">
                    Xá: <br className="block sm:hidden" />
                    <b className="text-slate-900">
                      {stats?.gingerBulkLiters || 0} L
                    </b>
                  </span>
                </div>
            ...
            {[
              {
                ...
                format: (v) => (typeof v === "number" ? `${v} loại` : "0 loại"),
                icon: Package,
              },
              {
                ...
                format: (v) =>
                  typeof v === "number" ? `${v.toLocaleString()} cái` : "0 cái",
                icon: ShoppingBag,
              },
              {
                ...
                format: (v) =>
                  typeof v === "number" ? `${v.toLocaleString()}đ` : "0đ",
                icon: TrendingUp,
              },
              {
              ...
              {
                label: "Cảnh báo cạn kho",
                value: stats?.lowStockCount,
                format: (v) =>
                  typeof v === "number" ? `${v} loại sản phẩm` : "0 loại",
                icon: AlertTriangle,
                alert: stats?.lowStockCount > 0,
              },
              {
                label: "Dự báo thông minh (AI)",
                value: stats?.lowStockCount === 0 ? "Ổn định" : "Cần nhập hàng",
                format: (v) => v,
                icon: Cpu,
              },
            ].map((card, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`rounded-[26px] border border-white/50 bg-white/20 p-5 shadow-soft backdrop-blur-md flex flex-col xs:flex-row items-center justify-between gap-4 text-center xs:text-left transition-colors ${
                    card.alert
                      ? "border-red-400 bg-red-500/5 shadow-red-200"
                      : ""
                  }`}
                >
                  <div className="space-y-1 flex flex-col items-center xs:items-start">
                  ...
                  <div
                    className={`h-11 w-11 rounded-2xl bg-white/70 shadow-sm flex items-center justify-center shrink-0 ${
                      card.alert
          ...
          <aside className="hidden xl:flex rounded-[36px] border border-white/50 bg-[#FAF6F0]/60 p-5 shadow-soft backdrop-blur-xl flex-col justify-between h-fit xl:h-[calc(100vh-6rem)] static xl:sticky top-24 min-w-[300px]">
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto space-y-3 pr-1 min-h-[300px]">
              {aiHistory.map((item, i) => (
                ...
                >
                  <div
                    className={`rounded-2xl px-4 py-2.5 shadow-sm text-xs leading-relaxed max-w-[88%] ${
                      item.role === "user"
              ...
              {aiLoading && (
                <div className="flex justify-start">
                  <div className="rounded-2xl rounded-tl-none bg-white border border-slate-200/50 px-4 py-2.5 shadow-sm text-xs text-slate-400 flex items-center gap-2">
                    <span className="animate-spin text-primary">⚡</span> AI
                    đang tính toán...
                  </div>
                </div>
              )}
            ...
            <div className="border-t border-slate-300/40 pt-3 mt-3">
              ...
              <div className="flex flex-col gap-1.5">
                {[
                  {
                    label: "Nhập 300ml bồ kết giá 85k",
                    query: "nhập 300ml bồ kết giá 85k",
                  },
                  {
                    label: "Xuất gừng 500ml 10 cái",
                    query: "xuất gừng 500ml 10 cái",
                  },
        ...
        {isChatOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-[380px] max-w-[95vw] z-50 bg-[#FAF6F0]/95 border-l border-white/40 shadow-2xl backdrop-blur-xl p-5 flex flex-col justify-between xl:hidden"
            >
              <div className="flex-1 flex flex-col min-h-0">
                {/* Header */}
                <div className="flex items-center justify-between border-b border-slate-300/40 pb-3 mb-3">
                  <div className="flex items-center gap-2">
                    <div className="rounded-xl bg-[#0f5132] p-2 text-white shadow-md">
                      <Sparkles size={16} />
                    </div>
                  ...
                  <button
                    onClick={() => setIsChatOpen(false)}
                    className="h-9 w-9 flex items-center justify-center rounded-xl bg-white/50 border border-slate-200 text-slate-700 hover:bg-slate-100/80 transition"
                  >
                    <X size={16} />
                  </button>
                </div>
                ...
                <div className="border-t border-slate-300/40 pt-3 mt-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {[
                      ...
                      {
                        label: "Dự báo & Restock kho",
                        query: "dự báo kho hàng",
                      },
                      { label: "Xuất báo cáo tổng hợp", query: "xuất báo cáo" },
                    ].map((act, i) => (
                      <button
